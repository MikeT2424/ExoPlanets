...
public class PlanetDAO {
	...
	public List<Planet> getPlanetList(@Min(5) double from, @Min(5) double to, String searchType) {
		...
		System.out.println("passed values to PlanetDAO.getPlanetList: to: " + to + " from " + from + " searchType: " + searchType); 
		...
		return  jdbc.query("SELECT * FROM allplanets where mass >=:from and mass <=:to", paramSource, new PlanetRowMapper());
		}
		else if(searchType.equals("radius")) {		
			return  jdbc.query("SELECT * FROM allplanets where radius >=:from and radius <=:to", paramSource, new PlanetRowMapper());
		}
		if(searchType.equals("period")) {		
			return  jdbc.query("SELECT * FROM allplanets where period >=:from and period <=:to", paramSource, new PlanetRowMapper());
			}
		if(searchType.equals("year")) {		
			return  jdbc.query("SELECT * FROM allplanets where year >=:from and year <=:to", paramSource, new PlanetRowMapper());
			}
		if(searchType.equals("star_distance")) {		
			return  jdbc.query("SELECT * FROM allplanets where star_distance >=:from and star_distance <=:to", paramSource, new PlanetRowMapper());
			}
		else if(searchType.equals("default")) {		
			return  jdbc.query("SELECT * FROM allplanets where ID >=:from and ID <=:to", paramSource, new PlanetRowMapper());
		}
		
		else return jdbc.query("Select * from allplanets", paramSource, new PlanetRowMapper());
	}

	public List<Planet> sortPlanets(List<Planet> planetList, String sort) {
if (sort.equals("sortIDr")) {
			
	System.out.println("called PlanetDAO.sortPlanets sort= " + sort);
	
			planetList.sort(Comparator.comparing(Planet::getID).reversed());
		} else if (sort.equals("sortIDa")) {
